...
const FilterActions:React.FC<FilterActionsInterface> = ({setFilterActionVisible}) => {  
  ...
  const onSetHeuristic:React.ChangeEventHandler<HTMLSelectElement> = (event) => setHeuristic(event.target.value); 
  ...
  return(
    <div ref={containerRef} className={styles['filter-actions']}>
      <label  className={styles['heuristic']}>
        <span>Heuristic</span>
          <Select name= "heuristicFilter" selectData={heuristics} severity={heuristic} onSetSelected={onSetHeuristic} />
      </label>
      <label className={styles['severity']}>
        <span>Severity</span>
          <Select name="severityFilter" selectData={severityData} severity={severity} onSetSelected={onSetSeverity} />
      </label>
    </div>
  );
}
...
const FilterTable = () => {
  ...
  return (
    <div className={styles["container"]}>
      <button  onClick={handleButtonFilterClick} className={`reset-btn ${styles['btn']} ${styles['active']}`}>
